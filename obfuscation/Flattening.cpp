...
void fixStack(Function &F) {
  ...
  do {
    ...
    for (unsigned int i = 0; i != tmpReg.size(); ++i) {
      DemoteRegToStack(*tmpReg.at(i), false, std::optional<BasicBlock::iterator>(F.begin()->getTerminator()->getIterator()));
    }

    for (unsigned int i = 0; i != tmpPhi.size(); ++i) {
      DemotePHIToStack(tmpPhi.at(i), std::optional<BasicBlock::iterator>(F.begin()->getTerminator()->getIterator()));
    }
